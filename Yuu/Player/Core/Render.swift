...
class Render {
    ...
    private func createBuffer(contentSize: CGSize ,viewBounds: CGRect) -> MTLBuffer? {
        ...
        let quadVertices: [SCVertex] = [SCVertex(position: SIMD2(Float(-1 * normalizedSamplingSize.width), Float(-1 * normalizedSamplingSize.height))),
                                      SCVertex(position: SIMD2(Float( 1 * normalizedSamplingSize.width), Float(-1 * normalizedSamplingSize.height))),
                                      SCVertex(position: SIMD2(Float(-1 * normalizedSamplingSize.width), Float( 1 * normalizedSamplingSize.height))),
                                      SCVertex(position: SIMD2(Float( 1 * normalizedSamplingSize.width), Float( 1 * normalizedSamplingSize.height)))]
        let vertexData = Data(bytes: quadVertices, count: MemoryLayout.size(ofValue: quadVertices))
